...
import { Search, MapPin, AlertTriangle, ExternalLink, Check, Sparkles, Layers } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const handleExport = () => {
    const headers = ["Business Name", "Website", "Phone Number", "Address"];
    const csvContent = [
      headers.join(","),
      ...
      <header className="sticky top-0 z-50 backdrop-blur-md bg-white/70 border-b border-white/20 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
          ...
          <div className="flex items-center gap-4">
             <div className="hidden md:flex items-center gap-1 px-3 py-1.5 bg-white/50 rounded-full border border-slate-200 shadow-sm text-xs font-semibold text-slate-600">
                <Sparkles className="w-3.5 h-3.5 text-accent" />
                <span>Lead Gen 2.0</span>
             </div>
          </div>
        </div>
      </header>
          ...
          <div className="animate-fade-in-up">
               ...
               <div className="relative group z-20">
                ...
                <form onSubmit={handleSearch} className="relative flex items-center">
                  ...
                  <input
                    ...
                    placeholder="e.g., 'Dentists in Manhattan'"
            ...
            {(places.length > 0 || isLoading) && (
              <div className="space-y-6 animate-fade-in-up" style={{animationDelay: '0.2s'}}>
                ...
                />
                <ResultsTable results={filteredPlaces} loading={isLoading} />
              </div>
            )}
          </div>
        )}
